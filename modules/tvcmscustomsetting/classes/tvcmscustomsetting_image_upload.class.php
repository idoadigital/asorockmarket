...
class TvcmsCustomSettingImageUpload extends Module
{
    ...
    public function imageUploading($image_src_1, $old_file)
    {
        ...
        /************resize settings***************/
        $savePath = _PS_MODULE_DIR_.'tvcmscustomsetting/views/img/';
        ...
        if ($resultType) {
            ...
            if ($resultUpload) {//success
                ...
                $res_2 = preg_match('/^App-logo-.*$/', $old_file);
                $res_3 = preg_match('/^pattern.*$/', $old_file);
                $res_4 = preg_match('/^no_pattern.*$/', $old_file);
