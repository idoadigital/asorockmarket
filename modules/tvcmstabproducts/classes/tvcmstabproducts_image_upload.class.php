...
class TvcmsTabProductsImageUpload extends Module
{
    public function imageUploading($image_src_1, $old_file)
    {
        ...
        /************resize settings***************/
        $savePath = _PS_MODULE_DIR_.'tvcmstabproducts/views/img/';
        ...
        if ($resultType) {
            ...
            if ($resultUpload) {//success
                $res1 = preg_match('/^main_offer_img_.*$/', $old_file);
                $res2 = preg_match('/^featured_offer_img_.*$/', $old_file);
                $res3 = preg_match('/^new_offer_img_.*$/', $old_file);
                $res4 = preg_match('/^best_seller_offer_img_.*$/', $old_file);
                $res5 = preg_match('/^special_offer_img_.*$/', $old_file);
