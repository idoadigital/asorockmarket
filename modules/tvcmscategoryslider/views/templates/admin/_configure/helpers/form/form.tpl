...
{block name="input"}
            ...
            {foreach from=$languages item=language}
                        ...
                        <div class="dummyfile input-group">
                            ...
                            <span class="input-group-btn">
                                <button id="{$input.name|escape:'htmlall':'UTF-8'}_{$language.id_lang|escape:'htmlall':'UTF-8'}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
                                    <i class="icon-folder-open"></i>{l s='Choose a file' mod='tvcmscategoryslider'}
                                </button>
                            </span>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    <div id="{$input.name|escape:'htmlall':'UTF-8'}-{$language.id_lang|escape:'htmlall':'UTF-8'|escape:'htmlall':'UTF-8'}-images-thumbnails" class="col-lg-12">
                        ...
                        <p class="help-block">{l s='Please Select Image.' mod='tvcmscategoryslider'} (Size:- {$width|escape:'htmlall':'UTF-8'} X {$height|escape:'htmlall':'UTF-8'})</p>
                    </div>
                </div>
    ...
    {if $input.type == 'tvcmscategory_select'}
        <div class="col-lg-9">
                ...
                <div class="col-lg-9">
                    <div class="dummyfile input-group">
                        <select class='tvcmscategory-slider-name' name="{$input.name|escape:'htmlall':'UTF-8'}">
                            {if $all_category}
                                <option value="0">{l s='Select Category' mod='tvcmscategoryslider'}</option>
                                ...
                                {/foreach}
                            {else}
                                <option value="0">{l s='No Category Found' mod='tvcmscategoryslider'}</option>
                            {/if}
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                <div id="{$input.name|escape:'htmlall':'UTF-8'}-{$language.id_lang|escape:'htmlall':'UTF-8'|escape:'htmlall':'UTF-8'}-images-thumbnails" class="col-lg-12">
                    <p class="help-block">{l s='Please select category.' mod='tvcmscategoryslider'} </p>
                </div>
            </div>
        </div>
    {/if}
