...
class TvcmsCategorySlider extends Module
{
    ...
    public function createVariable()
    {
        ...
        foreach ($languages as $lang) {
            $result['TVCMSCATEGORY_SLIDER_TITLE'][$lang['id_lang']] = 'Top Categories';
    ...
    public function selectAllLangIdById($id_tvcmscategory_slider)
    {
        $select_data = 'SELECT 
                        ...
                        WHERE 
                            ...
                            AND id_tvcmscategory_slider = '.$id_tvcmscategory_slider;
        $ans = Db::getInstance()->executeS($select_data);
        $return = array();
        foreach ($ans as $a) {
            $return[] = $a['id_lang'];
        ...
        if ($data['id']) {
            ...
            $insert_data[] = 'UPDATE `'._DB_PREFIX_.'tvcmscategory_slider`
                        ...
                        WHERE
                            `id_shop_group` = '.$this->id_shop_group.'
                            AND `id_shop` = '.$this->id_shop.'
                            AND `id_tvcmscategory_slider` = '.$id.';';
            ...
            $insert_data[] = 'INSERT INTO `'._DB_PREFIX_.'tvcmscategory_slider`
                        SET 
                            id_tvcmscategory_slider = '.$new_id.',
                            id_category = \''.$data['id_category'].'\',
                            position = '.$new_id.',
                            `id_shop_group` = '.$this->id_shop_group.',
                            `id_shop` = '.$this->id_shop.',
    ...
    public function showData($id = null)
    {
        ...
        $select_data .= 'SELECT * FROM `'._DB_PREFIX_.'tvcmscategory_slider` 
                ...
                .' AND `id_shop` = '.$this->id_shop;
        if ($id) {
            $select_data .= ' AND `id_tvcmscategory_slider` = '.$id;
        } else {
