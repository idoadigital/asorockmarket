...
{strip}
    <div class="tv-indicator tv-bar{if $useProgressiveColors} tv-colors{/if}">
        ...
        {if $page.page_name == 'module-tvcmsspecialproducts-default'}
        {if $isItemsDisplayable}
            <div class="tv-items">
                {if $hasMixedQty}
                    {if $productItems != 0} 
                        <span class="tvinstock"><span class="tvstock_tital">{l s='In Stock' mod='tvcmsstockinfo'}</span>
                        <b>{l s='Total items:' mod='tvcmsstockinfo'}{$productItems|escape:'html':'UTF-8'} </b>
                    {else}
                        <span class="tvinstock">{l s='No Product available' mod='tvcmsstockinfo'}
                    {/if}
                {/if}
        ...
        {if $isItemsDisplayable}
            <div class="tv-items">
                {if !$hasMixedQty}
                    ...
                    {else} 
                        <span class="tvinstock"><span class="tvstock_tital">{l s='In Stock ' mod='tvcmsstockinfo'}</span><b>{$productItems|escape:'html':'UTF-8'} </b>{l s='Available' mod='tvcmsstockinfo'}
                        {$stockIndicatorTrans.items|escape:'html':'UTF-8'}
                        </span>
                    {/if}
                {else}
