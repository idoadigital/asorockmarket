...
class TvcmsProductComment extends ObjectModel
{
    ...
    public static function getAveragesByProduct($id_product, $id_lang)
    {
        ...
        for ($i = 0; $i < $count_grades; ++$i) {
            if (array_key_exists($grades[$i]['id_tvcmsproduct_comment_criterion'], $criterionsGradeTotal) === false) {
    ...
    public function validate($validate = '1')
    {
        ...
        Hook::exec('actionObjectTvcmsProductCommentValidateAfter', array('object' => $this));
        return $success;
    }
