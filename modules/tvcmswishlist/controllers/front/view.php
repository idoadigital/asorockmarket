...
class TvcmsWishListViewModuleFrontController extends ModuleFrontController
{
    ...
    public function __construct()
    {
        ...
        include_once($this->module->getLocalPath().'tvcmswishlist.php');
    }
    ...
    public function initContent()
    {
        ...
        if ($token) {
            $wishlist = FunWishList::getByToken($token);

            FunWishList::refreshWishList($wishlist['id_wishlist']);
            ...
            FunWishList::incCounter((int)$wishlist['id_wishlist']);
            $ajax = Configuration::get('PS_BLOCK_CART_AJAX');

            $wishlists = FunWishList::getByIdCustomer((int)$wishlist['id_customer']);
        ...
        $this->setTemplate('module:tvcmswishlist/views/templates/front/view.tpl');
    }
}
