...
* @author PrestaShop SA <contact@prestashop.com>
    ...
    *}
    ...
    <div class="wishlistLinkTop">
        <a id="hideWishlist" class="button_account icon pull-right" href="#" onclick="WishlistVisibility('wishlistLinkTop', 'Wishlist'); return false;" rel="nofollow" title="{l s='Close this wishlist' mod='tvcmswishlist'}">
            <i class='material-icons'>&#xe15d;</i>
        </a>
        <ul class="clearfix display_list">
            <li>
                <a id="hideBoughtProducts" class="button_account" href="#" onclick="WishlistVisibility('wlp_bought', 'BoughtProducts'); return false;" title="{l s='Hide products' mod='tvcmswishlist'}">
                    {l s='Hide products' mod='tvcmswishlist'}
                </a>
                <a id="showBoughtProducts" class="button_account" href="#" onclick="WishlistVisibility('wlp_bought', 'BoughtProducts'); return false;" title="{l s='Show products' mod='tvcmswishlist'}">
                    {l s='Show products' mod='tvcmswishlist'}
                </a>
            </li>
            {if count($productsBoughts)}
            <li>
                <a id="hideBoughtProductsInfos" class="button_account" href="#" onclick="WishlistVisibility('wlp_bought_infos', 'BoughtProductsInfos'); return false;" title="{l s='Hide products' mod='tvcmswishlist'}">
                    {l s='Hide bought products info' mod='tvcmswishlist'}
                </a>
                <a id="showBoughtProductsInfos" class="button_account" href="#" onclick="WishlistVisibility('wlp_bought_infos', 'BoughtProductsInfos'); return false;" title="{l s='Show products' mod='tvcmswishlist'}">
                    {l s='Show bought products info' mod='tvcmswishlist'}
                </a>
            </li>
            {/if}
        </ul>
        <p class="wishlisturl form-group">
            <label>{l s='Permalink' mod='tvcmswishlist'}:</label>
            <input type="text" class="form-control" value="{$link->getModuleLink('tvcmswishlist', 'view', ['token' => $token_wish])|escape:'html':'UTF-8'}" readonly="readonly" />
        </p>
        <div class="submit">
            <div id="showSendWishlist">
                <a class="tvall-inner-btn" href="#" onclick="WishlistVisibility('wl_send', 'SendWishlist'); return false;" title="{l s='Send this wishlist' mod='tvcmswishlist'}">
                    <span>{l s='Send this wishlist' mod='tvcmswishlist'}</span>
                </a>
            </div>
        </div>
        {/if}
        ...
        {if !$refresh}
        <form method="post" id="wishlist_form" class="wl_send unvisible" onsubmit="return (false);">
            <a id="hideSendWishlist" class="button_account btn icon" href="#" onclick="WishlistVisibility('wl_send', 'SendWishlist'); return false;" rel="nofollow" title="{l s='Close this wishlist' mod='tvcmswishlist'}">
                <i class='material-icons'>&#xe5cd;</i>
            </a>
            <fieldset>
                <div class="required form-group">
                    <label class="col-xs-12 col-md-2 col-lg-2 tvwishlist-vuew-label" for="email1">{l s='Email' mod='tvcmswishlist'}1<sup>*</sup></label>
                    <div class="col-xs-12 col-md-10 col-lg-10">
                        <input type="email" name="email1" id="email1" class="form-control" required="true"/>
                    </div>
                </div>
                {section name=i loop=11 start=2}
                <div class="form-group">
                    <label class="col-xs-12 col-md-2 col-lg-2 tvwishlist-vuew-label" for="email{$smarty.section.i.index|escape:'htmlall':'UTF-8'}">{l s='Email' mod='tvcmswishlist'}{$smarty.section.i.index|escape:'htmlall':'UTF-8'}</label>
                    <div class="col-xs-12 col-md-10 col-lg-10">
                        <input type="email" name="email{$smarty.section.i.index|escape:'htmlall':'UTF-8'}" id="email{$smarty.section.i.index|escape:'htmlall':'UTF-8'}" class="form-control" />
                    </div>
                </div>
                {/section}
                <div class="col-xs-12 col-md-0 col-lg-2 tvwishlist-inner-page-send"></div>
                <p class="required col-xs-6 col-md-6 col-lg-2 tvwishlist-required">
                     {l s='Required field' mod='tvcmswishlist'}<sup>*</sup>
                </p>
                <div class="submit col-xs-6 col-md-6 col-lg-8 tvwishlist-send-mess-btn">
                    <button class="tvall-inner-btn" type="submit" name="submitWishlist" onclick="WishlistSend('wl_send', '{$id_wishlist|escape:'htmlall':'UTF-8'}', 'email');">
                        <span>{l s='Send' mod='tvcmswishlist'}</span>
                    </button>
                </div>
            </fieldset>
        </form>
        {* {if count($productsBoughts)}
        <table class="wlp_bought_infos unvisible table table-bordered table-responsive tvcmswishlist-view-table-data">
            <thead>
                <tr>
                    <th class="first_item">{l s='Product' mod='tvcmswishlist'}</th>
                    <th class="item">{l s='Quantity' mod='tvcmswishlist'}</th>
                    <th class="item">{l s='Offered by' mod='tvcmswishlist'}</th>
                    <th class="last_item">{l s='Date' mod='tvcmswishlist'}</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td class="first_item">
                        <span style="float:left;">
                            <img src="{$link->getImageLink($product.link_rewrite, $product.cover, 'small_default')|escape:'html':'UTF-8'}" alt="{$product.name|escape:'html':'UTF-8'}" />
                        </span>
                        <span style="float:left;">
                    ...
                    <td class="item align_center">
                        {$bought.quantity|intval|escape:'htmlall':'UTF-8'}
                    </td>
                    <td class="item align_center">
                        {$bought.firstname|escape:'htmlall':'UTF-8'} {$bought.lastname|escape:'htmlall':'UTF-8'}
                    </td>
                    <td class="last_item align_center">
                        {$bought.date_add|date_format:"%Y-%m-%d"|escape:'htmlall':'UTF-8'}
                    </td>
                </tr>
                {/if}
                {/foreach}
